...
public class LoanRepaymentService {
    ...
    public ApiResponse<LoanRepaymentResponse> makeRepayment(LoanRepaymentRequest request) {
        ...
        List<LoanSchedule> pendingEmis =
                loanScheduleRepository.findNextPendingEmis(loan.getId());

        if (pendingEmis.isEmpty()) {
            throw new InvalidRequestException("No pending EMI found");
        }

        LoanSchedule nextEmi = pendingEmis.get(0);
        ...
        long pendingEmiCount = loanScheduleRepository.countPendingEmis(loan.getId());
        if (pendingEmiCount == 0L || newRemainingPrincipal.compareTo(BigDecimal.ZERO) == 0) {
